...
//            System.out.println(">> args is " + args[0] + " " + args[1] + " " + args[2]);
            ...
            for (int i = 0; i < listOfFiles.length; i++) {
                executor.execute(new Task(args));
            }
            executor.shutdown();
...
class Task implements Runnable {
    ...
    public void run() {
        ...
        File[] listOfFiles = folder.listFiles();
        for (int i = 0; i < listOfFiles.length; i++) {
            args[0] = listOfFiles[i].getPath();
            args[2] = listOfFiles[i].getName();
            try {
                patternChecker(args);
            } catch (Exception e) {
                e.printStackTrace();
...
//            String seconds = df.format(convertSeconds);
        if (isPDF) {
            System.out.println(args[2]  + ":" + " " + "PDF document");
//                System.out.println("It took" + " " + seconds + " " + "seconds");
        }
        else if (isDOC) {
            System.out.println(args[2] + ":" + " " + "DOC document");
        }
        else if (isPEM) {
            System.out.println(args[2] + ":" + " " + "PEM certificate");
        } else {
            System.out.println(args[2] + ":" + " " + "Unknown file type");
        }
